...
from dataloader import ParquetDataLoader  # 导入Parquet数据加载器
import sys
import os
...
def train_and_compare(ticker='AAPL', lookback=30, epochs=50, batch_size=32, out_dir='models', period='5y'):
    ...
    cp_lstm = ModelCheckpoint(os.path.join(
        out_dir, 'best_lstm.h5'), monitor='val_loss', save_best_only=True)
    cp_gru = ModelCheckpoint(os.path.join(
        out_dir, 'best_gru.h5'), monitor='val_loss', save_best_only=True)
...
if __name__ == "__main__":
    ...
    result = train_and_compare(
        ticker='AAPL', lookback=30, epochs=50, period='5y')
    print("训练结果：")
